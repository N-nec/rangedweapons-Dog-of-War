...
rangedweapons_rocket.on_step = function(self, dtime, pos)
	...
	local objs = minetest.get_objects_inside_radius({x = pos.x, y = pos.y, z = pos.z}, 1.5)
		...
		if obj:get_luaentity().name ~= "rangedweapons:rocket" --and obj:get_hp() > 0 and not minetest.is_protected(pos, "")
		then
...
rangedweapons_he_grenade.on_step = function(self, dtime, pos)
	...
	local objs = minetest.get_objects_inside_radius({x = pos.x, y = pos.y, z = pos.z}, 1.5)
		for k, obj in pairs(objs) do
		if obj:get_luaentity() ~= nil then
		if obj:get_luaentity().name ~= "rangedweapons:he_grenade" and obj:get_hp() > 0 --and not minetest.is_protected(pos, "")
		then
