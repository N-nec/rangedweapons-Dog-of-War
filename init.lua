...
p_meta:set_int(skill, skill_num + 1)
   -- minetest.chat_send_player(player:get_player_name(), "" ..core.colorize(""))
end
end
...
----------------------------------
		...
		if pos and dir and yaw then
		minetest.sound_play(shoot_sound, {pos = pos, max_hear_distance = 40})
		pos.y = pos.y + 1.5
		--pos.x = pos.x + 0.5
		--player:set_fov(1, true, 0)
		if controls.sneak or controls.zoom then
		pos.y = pos.y - 1
		--player:set_fov(1, true, 0)
		end
...
dofile(modpath.."/The_Needle.lua")
dofile(modpath.."/The_Man.lua")
dofile(modpath.."/shotfun.lua")
dofile(modpath.."/The_Sniper.lua")
dofile(modpath.."/The_Dog.lua")
dofile(modpath.."/Stone.lua")
--dofile(modpath.."/chord.lua")
...
----[[ What is this good for? ### ABM to remove laggy fire ###
minetest.register_abm({
	nodenames = {"fire:basic_flame"},
	interval = 1,
	chance = 1,
	action = function(pos, node)
		if minetest.get_node(pos).name == "fire:basic_flame" then
			node.name = "air"
...
rangedweapons_empty_shell.on_step = function(self, dtime, pos)
	...
	self.object:set_velocity({x=vel.x*-0.3, y=vel.y*-0.75, z=vel.z*-0.3})
			minetest.sound_play("rangedweapons_shellhit", {pos = self.lastpos, gain = 0.3})
	self.object:set_acceleration({x=acc.x, y=acc.y, z=acc.z})
			end end
	end
	if self.timer > 1 then
			minetest.sound_play("rangedweapons_bulletdrop", {pos = self.lastpos, gain = 0.3})
		self.object:remove()
